...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('BOT NGNTD', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('ZFA', 'yellow'))
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hallo kak~, Aku Raisa BOT, Kakak bisa panggil aku Sasa. Untuk mencari tahu command BOT, ketik ${prefix}menu Yah Kak`)
        })
	    }
	}
    })
   ...
   aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'remove' && event.who !== host) {
            ...
            const aigo = `eh @${event.who.replace('@c.us', '')} semoga tenang di alam sana al-fatihah:(`
